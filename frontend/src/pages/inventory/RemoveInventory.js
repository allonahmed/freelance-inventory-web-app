...
import { removeColumns } from "../../components/table/columns";
...
const RemoveInventory = () => {
  ...
  const newList = updatedList(itemList).filter((item) => {
    ...
    return item.current_location === vehicle;
  });

  const type = "remove";
  removeColumns[0]["Header"] = "Choose items to remove from " + vehicle;

  const column = useMemo(() => removeColumns);
  return (
